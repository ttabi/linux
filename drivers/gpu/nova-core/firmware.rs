...
fn get_signature_section(chipset: Chipset) -> Result<&'static str> {
    match chipset.arch() {
        ...
        gpu::Architecture::Blackwell => {
            // Distinguish between GB10x and GB20x series like Nouveau does
            match chipset {
                ...
                // Future GB10x series chips would go here
                // For now, all other Blackwell chips default to gb10x
                _ => Ok(".fwsignature_gb10x"),
            }
        }
        _ => Err(ENOTSUPP),
    }
