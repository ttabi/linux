...
const FSP_MSG_TIMEOUT_MS: i64 = 1000;
...
// and other structs that are also AsBytes/FromBytes
...
impl Fsp {
    ...
    /// Wait for FSP to have data available.
    ///
    /// References Nouveau's gh100_fsp_wait() implementation.
    /// Uses simple polling loop to match Nouveau's behavior exactly.
    fn wait_fsp(dev: &device::Device<device::Bound>, bar: &Bar0) -> Result<()> {
        // Reduced logging - only log at start and end to avoid spam
        dev_dbg!(
            dev,
            "FSP waiting for response (timeout: {}ms)\n",
            FSP_MSG_TIMEOUT_MS
        );

        // Add initial state debugging (but not frequent polling)
        ...
        loop {
            if Self::poll_fsp(dev, bar) > 0 {
                dev_dbg!(
                    dev,
                    "FSP response detected after {}ms\n",
    ...
    fn send_fsp(
        ...
        // Write message to EMEM using falcon PIO write with logging
    ...
    /// Receive response from FSP via falcon EMEM.
    ///
    ...
    fn recv_fsp(
        ...
        let packet_size = Self::poll_fsp(dev, bar) as usize;
        ...
        // Read response from EMEM using falcon PIO read with logging
        let mut packet = KVec::with_capacity(packet_size, GFP_KERNEL)?;
        packet.resize(packet_size, 0u8, GFP_KERNEL)?;
        fsp_falcon.read_emem(dev, bar, 0, &mut packet)?;

        // Reset message queue pointers after reading (matches Nouveau)
    ...
    /// Send FSP message and wait for synchronous response.
    ///
    ...
    fn send_sync_fsp(
        ...
        // Wait for response
        Self::wait_fsp(dev, bar)?;

        // Receive response
        let response_data = Self::recv_fsp(dev, bar, fsp_falcon, size_of::<FspResponse>())?;
